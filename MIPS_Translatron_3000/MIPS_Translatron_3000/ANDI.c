...
void andi_immd_assm(void) {
	...
	*/
	...
	// First Parameter needs to be a register
	if (PARAM1.type != REGISTER) {
		state = MISSING_REG;
		return;
	}

	// Second Parameter needs to be a register
	if (PARAM2.type != REGISTER) {
		state = MISSING_REG;
		return;
	}

	// This is an immediate function, so the second parameter needs to be an immediate
